...
namespace OpenCover.UI.Processors
{
    ...
    internal class Discoverer
    {
        ...
        /// </summary>
        /// <param name="discoveryDone">
        /// The delegate that needs to be called after test discovery is done.
        /// </param>
        ...
        private void StartTestDiscovery(Action<List<TestClass>> discoveryDone, string testDiscovererPath, string testsDLLs)
        {
            ...
            var nunitPathEncoded = Convert.ToBase64String(Encoding.UTF8.GetBytes(OpenCoverUISettings.Default.NUnitPath)); // requires encoding because of Pipe interface bahaviour

            var processInfo = new ProcessStartInfo(testDiscovererPath, String.Format("{0} {1} {2}", pipeGuid, nunitPathEncoded, testsDLLs))
            {
                CreateNoWindow = true,
